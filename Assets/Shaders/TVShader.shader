...
Shader "Custom/VHSeffect"
{
    Properties
    {
		...
		_GoTan ("Use Tan UV Offset", float) = 0
    }
    SubShader
    {
        Pass
        {
            ...
            {
				float x = frac(i.uv.x + sin((i.uv.y + _CosTime.y) * 100) / _OffsetDistortion) * (1 - _GoTan) + 
					frac(i.uv.x + tan((i.uv.y + _CosTime.y) * 100) / _OffsetDistortion) * _GoTan;
                i.uv = float2(x, frac(i.uv.y + _OffsetPosY));
